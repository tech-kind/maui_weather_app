using MauiWeatherApp.Constants;
...
public class WeatherDataRepository : IWeatherDataRepository
{
    ...
    {
        return await GetAsyncMethodFactory<List<Hourly>>(TypeMethod.GetHouriesAsync);
    }
    ...
    {
        Location location = await Geolocation.Default.GetLocationAsync();
        Uri url = new(GenerateRequestUri(APIConstants.OpenWeatherMapEndpoint, location.Longitude, location.Latitude));
    ...
    public static T GetDeserializedContent<T>(TypeMethod typeMethod, string content) where T : class
    {
        switch (typeMethod)
        {
            ...
            case TypeMethod.GetHouriesAsync:
